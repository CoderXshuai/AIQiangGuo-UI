 ...
 try{//20201025使用try catch(e)语句处理错误，去除前后置0.5s延时   
  ...
  if (!className("RadioButton").exists() || className("android.view.View").text("继续挑战").exists() || textContains("继续挑战").exists() /*|| !textContains("距离答题结束").exists()*/){//不存在本局结束标志 继续挑战，则执行  20201225界面变化，距离答题结束 删除
    ...
    if (className("RadioButton").exists() || aquestion.length == 0) {
        /*delay(0.3);*/
        let aControl = className("ListView").findOnce().parent().children();//题目控件对象，用于提取多控件题目
        // console.info("控件数量：" + aControl.length +"题目：" +aControl[0].text()); //[0]--->题目,[1],[2],[3]--->其它题目控件
       var aquestion;
     if ( aControl.length > 4){
          for (var i = 0; i < 5;i++)
         { aquestion += aControl[i].text(); }
          //console.info("发现6控件题目"); 
       }else{
       aquestion = aControl[0].text(); 
       } 
        /*var aquestion = className("ListView").findOnce().parent().child(0).text();*/ //单控件题目提取
        ...
        while (aquestion == oldaquestion || question == "") {
         ...
         aControl = className("ListView").findOnce().parent().children();//题目控件对象，用于提取多控件题目
         if ( aControl.length > 4){
          for (var i = 0; i < 5;i++)
         { aquestion += aControl[i].text(); }
          }else{
         ...
         /*aquestion = className("ListView").findOnce().parent().child(0).text();*/ //单控件题目提取
