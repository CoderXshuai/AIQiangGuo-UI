...
function start_app() {
  ...
  console.log("Github地址  https://github.com/zhuxuedefeng/AiQiangGUO");
  console.log("网页聊天室 https://gitter.im/AutoJSQiangGuo/community");
  console.log("网盘下载地址 https://wws.lanzous.com/b01npbw7a 密码:263a");
  delay(3);
  console.log("启动学习强国");
  ...
  delay(4); //如果已清理强国app后台，默认打开主页;如果未清理后台，3秒应该可以拉起强国app  
...
function getScores() {
  ...
  while (!err) {
    try {
      // console.log(className("android.widget.ListView").findOnce().children().toString());
...
function sub2() {
  ...
  delay(0.5)
  click("强国号", 0)
  // console.log(className("android.view.View").scrollable(true).depth(14).findOne().parent());
  let sublist = className("android.view.View").scrollable(true).depth(14).findOne().children();
  // let sublist = className("ListView").findOnce();
  var i = 0;
  console.log(sublist.size());
  while (i < asub) {
    let object = desc("订阅").find();
    if (sublist.size() >= asub) {
      // sublist.forEach(function (currentValue) {
      //   if (currentValue && i < asub) {
      //     let like = currentValue.parent()
      //     if (like.click()) {
      //       console.log("订阅成功");
      ...
      //   }
      // })
      // sublist.children().forEach(function (item) {
      //   item.child(2).click();
      //   console.log("订阅成功");
      // })
      sublist[i].child(2).click();
      console.log('第' + (i + 1) + '次订阅成功');
      i++;
    } else if (text("你已经看到我的底线了").exists()) {
      console.log("尝试订阅学习平台")
      back();
      ...
      click("学习平台", 0);
      delay(2);
      let sublist = className("android.view.View").scrollable(true).depth(14).findOne();
      while (i < asub) {
        let object = desc("订阅").find();
        if (!object.empty()) {
          object.forEach(function (currentValue) {
